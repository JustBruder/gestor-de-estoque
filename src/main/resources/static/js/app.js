...
// ==========================================
// SEÇÃO: INSUMOS (ESTOQUE BASE & ESTOQUE BAIXO)
// ==========================================
...
async function carregarInsumos() {
    try {
        ...
        // Renderiza na tabela de insumos geral
        ...
        // Filtra e renderiza o Estoque Baixo automaticamente (< 20 para UN, < 200 para Gramas/ML)
        renderizarEstoqueBaixo(ingredientes);
...
function renderizarEstoqueBaixo(ingredientes) {
    const containerBaixo = document.getElementById("containerEstoqueBaixo");
    const painelBaixo = document.getElementById("painelEstoqueBaixo");
    if (!containerBaixo) return;

    const itensBaixos = ingredientes.filter(ing => {
        const qtd = ing.quantidadeEstoque !== undefined ? ing.quantidadeEstoque : (ing.quantidade || 0);
        const unidade = (ing.unidadeMedida || "").toUpperCase();
        
        if (unidade.includes("GRAMA") || unidade.includes("ML") || unidade.includes("G")) {
            return qtd <= 200;
        } else {
            return qtd <= 20;
        }
    ...
    if (itensBaixos.length === 0) {
        if (painelBaixo) painelBaixo.style.display = "none"; // Oculta o alerta se estiver tudo normal
        containerBaixo.innerHTML = `<p style="color: #aaa; font-size: 0.9em; margin: 0;">Nenhum item em estoque baixo no momento.</p>`;
    } else {
        if (painelBaixo) painelBaixo.style.display = "block";
        containerBaixo.innerHTML = itensBaixos.map(ing => {
            const qtd = ing.quantidadeEstoque !== undefined ? ing.quantidadeEstoque : (ing.quantidade || 0);
            return `
                <div style="display: flex; justify-content: space-between; align-items: center; background: rgba(0,0,0,0.3); padding: 8px 12px; border-radius: 6px;">
                    <span style="color: #fff; font-weight: bold;">${ing.nome}</span>
                    <span style="color: #e74c3c; font-weight: bold;">Restam: ${qtd} ${ing.unidadeMedida || 'UN'}</span>
                </div>
            `;
        }).join('');
    }
